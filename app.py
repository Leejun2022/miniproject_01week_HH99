...
def comment_get():
    comment_list = list(db.comments.find({}, {'_id': False}).sort("date", -1))
    return jsonify({'comments': comment_list})

@app.route("/mycomment", methods=["GET"])
def my_comment_get():
    username_receive = request.args.get("username_give")
    if username_receive == '':
        comments_list = list(db.comments.find({}, {'_id': False}).sort("date", -1))
    else:
        comments_list = list(db.comments.find({"username": username_receive}, {'_id': False}).sort("date", -1))
    return jsonify({"result": "success", "msg": "댓글을 가져왔습니다.", "mycomments": comments_list})
